...
def getDeployedSecretV1(dongle, masterPrivate, targetId):
    ...
    while True:
        ...
        certificateSignature = last_pub_key.ecdsa_deserialize(
            bytes(certificate[2 + certificate[0] :])
        )
        if not last_pub_key.ecdsa_verify(
            bytes(certificatePublic), certificateSignature
        ):
            if index == 0:
...
def getDeployedSecretV2(
    dongle, masterPrivate, targetId, signerCertChain=None, ecdh_secret_format=None
):
    testMaster = PrivateKey(bytes(masterPrivate))
    testMasterPublic = bytearray(testMaster.pubkey.serialize(compressed=False))
    targetid = bytearray(struct.pack(">I", targetId))
    ...
    while True:
        ...
        certificateSignature = last_dev_pub_key.ecdsa_deserialize(
        ...
        if not last_dev_pub_key.ecdsa_verify(
