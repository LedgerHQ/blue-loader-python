...
def SCPv3(
    ...
    if signer_private_key is not None:
        ...
        if user_mode:
            ...
            certificate_id = 0x01
        certificate = (
            bytearray([len(signer_pk)])
            + signer_pk
            + bytearray([len(signature)])
            + signature
        )
        apdu = (
            bytearray([0xE0, 0x51, 0x00, certificate_id])
    ...
    data_to_sign = bytes(
        bytearray([ephemeral_role]) + nonce + device_nonce + ephemeral_public
    )
    signature = signer_sk.ecdsa_sign(bytes(data_to_sign))
    signature = signer_sk.ecdsa_serialize(signature)
    certificate = (
        bytearray([len(ephemeral_public)])
        + ephemeral_public
        + bytearray([len(signature)])
        + signature
    )
    apdu = (
        bytearray([0xE0, 0x51, 0x80, certificate_id])
        + bytearray([len(certificate)])
        + certificate
    )
    ...
    (
        ...
        certificate_static_public_key,
        certificate_static_signature_array,
    ) = extract_from_certificate(certificate_static)
    certificate_signature = issuer_pk.ecdsa_deserialize(
        bytes(certificate_static_signature_array)
    )
    certificate_signed_data = (
        bytearray([CERT_ROLE_DEVICE])
        + certificate_static_header
        + certificate_static_public_key
    )

    if not issuer_pk.ecdsa_verify(
        bytes(certificate_signed_data), certificate_signature
    ):
    ...
    certificate_signature = device_pub_key.ecdsa_deserialize(
        bytes(certificate_signature_array)
    )
    certificate_signed_data = (
        bytearray([CERT_ROLE_DEVICE_EPHEMERAL])
        + device_nonce
        + nonce
        + certificate_public_key
    )

    if not device_pub_key.ecdsa_verify(
        bytes(certificate_signed_data), certificate_signature
    ):
...
def recoverValidate(loader, caKey, name, staticPrivateKey):
    ...
    dataToSign = bytes(
        ...
        + name.encode()
        + struct.pack(">B", len(providerPublicKey))
        + providerPublicKey
    )
    ...
    loader.recoverValidateCertificate(
        bytes(version), bytes(role), name, providerPublicKey, signature
    )
    ...
    dataToSign = bytes(
        ...
        + struct.pack(">B", len(name))
        + name.encode()
        + struct.pack(">B", len(ephemeralPublic))
        + ephemeralPublic
    )
    ...
    loader.recoverValidateCertificate(
        bytes(version), bytes(role), name, ephemeralPublic, signature, True
    )
