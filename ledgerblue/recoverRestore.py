...
def get_argparser():
    ...
    parser.add_argument(
        "-s",
        "--select",
        help="Select the backups to use",
        required=True,
        action="store",
        dest="backups",
        type=str,
    ...
    parser.add_argument(
        "--gpg",
        help="Decrypt the backup data. Enter the email address associated to your gpg key",
        default=None,
        action="store",
    )
    return parser
...
if __name__ == "__main__":
    ...
    for backup in args.backups:
        ...
        shareCommit = recoverSession.recoverShareCommit(
            bytearray.fromhex(point), bytearray.fromhex(share)
        )

        # Validate share's commitment
        recoverSession.recoverValidateCommit(
            loader, bytearray.fromhex(commitments), shareCommit
        )

        # Send the share to the device (encrypted with the device-provider shared key)
        recoverSession.recoverRestoreSeed(
            loader, bytearray.fromhex(shareAndIndex), numberOfWords
        )
