...
def get_argparser():
    parser = argparse.ArgumentParser(
        description="""Read a sequence of command APDUs from a file and send them to the
device. The file must be formatted as hex, with one CAPDU per line."""
    ...
    parser.add_argument(
        "--scp", help="Open a Secure Channel to exchange APDU", action="store_true"
    )
    parser.add_argument(
        "--targetId",
        help="The device's target ID (default is Ledger Nano S). If --elfFile is used, the targetId from the ELF file will be used instead.",
        type=auto_int,
        default=0x31100002,
    )
    parser.add_argument(
        "--elfFile",
        help="ELF file from which the target ID is fetched. Overrides '--targetId'",
    )
    parser.add_argument(
        "--rootPrivateKey",
...
if __name__ == "__main__":
    ...
    for data in file:
        ...
        if args.scp:
            ...
            apdu = bytearray(
                [data[0], data[1], data[2], data[3], len(apduData)]
            ) + bytearray(apduData)
