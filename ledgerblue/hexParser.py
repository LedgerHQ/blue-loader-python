    ...
    def __init__(self, fileName):
        ...
        for data in file:
            ...
            if recordType == 0x00:
                if startZone == None:
                    raise Exception(
                        "Data record but no zone defined at line " + str(lineNumber)
                    )
                if startFirst == None:
                    startFirst = address
                    current = startFirst
                if address != current:
                    self._addArea(
                        IntelHexArea((startZone << 16) + startFirst, zoneData)
                    )
                    zoneData = b""
                    startFirst = address
                    current = address
...
class IntelHexPrinter:
    ...
    def _emit_binary(self, file, bin):
        ...
        s = (
            ":"
            + binascii.hexlify(bin).decode("utf-8")
            + hex(0x100 + cks)[3:]
            + self.eol
        ).upper()
        if file != None:
    ...
    def writeTo(self, fileName, blocksize=32):
            ...
            while off < len(area.data):
                ...
                if (off & 0xFFFF0000) != (oldoff & 0xFFFF0000):
                    self._emit_binary(
                        file,
                        bytearray.fromhex(
                            "02000004" + hex(0x10000 + (area.start >> 16))[3:7]
                        ),
                    )
                ...
                if off + blocksize > len(area.data):
                    self._emit_binary(
                        file,
                        bytearray.fromhex(
                            hex(0x100 + (len(area.data) - off))[3:]
                            + hex(0x10000 + off + (area.start & 0xFFFF))[3:]
                            + "00"
                        )
                        + area.data[off : len(area.data)],
                    )
                else:
                    self._emit_binary(
                        file,
                        bytearray.fromhex(
                            hex(0x100 + blocksize)[3:]
                            + hex(0x10000 + off + (area.start & 0xFFFF))[3:]
                            + "00"
                        )
                        + area.data[off : off + blocksize],
                    )
        ...
        s = (
            ":04000005"
            + bootAddrHex
            + hex(0x100 + self.checksum(bytearray.fromhex("04000005" + bootAddrHex)))[
                3:
            ]
            + self.eol
        )
