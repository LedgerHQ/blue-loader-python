...
class IntelHexParser:
    ...
    def __init__(self, fileName):
        ...
        for data in file:
            ...
            if recordType == 0x05:
                self.bootAddr = ((data[4] & 0xFF) << 24) + ((data[5] & 0xFF) << 16) + ((data[6] & 0xFF) << 8) + (
                        data[7] & 0xFF)
        file.close()
        ...
        for area in self.areas:
            ...
            while off < len(area.data):
                ...
                if (off & 0xFFFF0000) != (oldoff & 0xFFFF0000):
                    self._emit_binary(file,
                                      bytearray(("02000004" + hex(0x10000 + (area.start >> 16))[3:7]).decode('hex')))

                # emit data record
                if off + blocksize > len(area.data):
                    self._emit_binary(file, bytearray((hex(0x100 + (len(area.data) - off))[3:] + hex(
                        0x10000 + off + (area.start & 0xFFFF))[3:] + "00").decode('hex')) + area.data[
                                                                                            off:len(area.data)])
                else:
                    self._emit_binary(file, bytearray(
                        (hex(0x100 + blocksize)[3:] + hex(0x10000 + off + (area.start & 0xFFFF))[3:] + "00").decode(
                            'hex')) + area.data[off:off + blocksize])

                oldoff = off
                off += blocksize

        bootAddrHex = hex(0x100000000 + self.bootAddr)[3:]
        s = ":04000005" + bootAddrHex + hex(0x100 + self.checksum(bytearray(("04000005" + bootAddrHex).decode('hex'))))[
                                        3:] + self.eol
        if file is not None:
            file.write(s)
        else:
