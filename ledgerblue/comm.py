...
TCP_PROXY = None
if (
    "LEDGER_PROXY_ADDRESS" in os.environ
    and len(os.environ["LEDGER_PROXY_ADDRESS"]) != 0
    and "LEDGER_PROXY_PORT" in os.environ
    and len(os.environ["LEDGER_PROXY_PORT"]) != 0
):
    TCP_PROXY = (
...
class HIDDongleHIDAPI(Dongle, DongleWait):
    ...
    def exchange(self, apdu, timeout=TIMEOUT):
        ...
        if sw != 0x9000 and (sw & 0xFF00) != 0x6100 and (sw & 0xFF00) != 0x6C00:
            ...
            raise CommException(
                "Invalid status %04x (%s)" % (sw, possibleCause), sw, response
            )
        return response
