...
def get_argparser():
    parser = argparse.ArgumentParser(
        description="Delete the custom Certificate Authority used to "
        "backup/restore a seed."
    )
    parser.add_argument(
        "--targetId", help="The device's target ID (default is Nano X)", type=auto_int
    )
    parser.add_argument("--name", help="The certificate's name", required=True)
    parser.add_argument(
        "--issuerPublicKey",
    ...
    parser.add_argument(
        "--rootPrivateKey",
        help="The private key of the Signer used to establish a secure channel "
        "(otherwise a random one will be generated)",
    )
    parser.add_argument(
        "--caPublicKey",
        help="The Custom CA's public key to be enrolled (hex encoded)",
        required=True,
    )
    return parser
...
if __name__ == "__main__":
    ...
    # Delete the Certificate Authority's public key
    loader.recoverDeleteCA(args.name, publicKey)
