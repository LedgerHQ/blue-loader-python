...
def get_argparser():
    ...
    parser.add_argument("--issuerPublicKey", help="The public key of the Issuer (used to verify the certificate of "
                                                  "the device)")
    parser.add_argument("--numberOfWords", help="The number of words in the mnemonic (default is 24 words)", type=auto_int)
    parser.add_argument("-c", "--configuration", help="Configuration file", default=None, required=True, action="store")
    parser.add_argument("--gpg", help="Encrypt the backup data. Enter the email address associated to your gpg key",
...
if __name__ == '__main__':
    ...
    secret, devicePublicKey = SCPv3(dongle, bytearray.fromhex(args.issuerPublicKey), args.targetId,
                                    bytearray.fromhex(args.rootPrivateKey), bytearray.fromhex(orchestratorPrivateKey))
    ...
    for provider in conf['providers']:
        ...
        share, idx, deletePublicKey, commitHash = recoverSession.recoverBackupProviderDecrypt(encryptedData)
        ...
        result, shareCommit = recoverSession.recoverVerifyCommitments(share, idx, commitments, commitmentPoint)
        if result:
            # Do the backup
            backup_data[name] = dict()
            backup_data[name]['share'] = decode_bytes(binascii.hexlify(share))
            backup_data[name]['index'] = decode_bytes(binascii.hexlify(idx.to_bytes(4, 'little')))
            backup_data[name]['commitments'] = decode_bytes(binascii.hexlify(commitments))
            backup_data[name]['share_commit'] = decode_bytes(binascii.hexlify(shareCommit))
            ...
            delete_data[name]['public_key'] = decode_bytes(binascii.hexlify(deletePublicKey))
        else:
            ...
            try:
                if args.gpg:
                    with open(name + ".json.gpg",'w') as file:
                        encode_backup_data = json.dumps(backup_data, indent = 4, sort_keys = True).encode("utf-8")
                        encrypted_backup = gpg.encrypt(encode_backup_data,recipients=[args.gpg])
                        file.write(str(encrypted_backup))
                    with open("Delete" + name + ".json.gpg",'w') as file:
                        encode_delete_data = json.dumps(delete_data, indent = 4).encode("utf-8")
                        encrypted_data = gpg.encrypt(encode_delete_data,recipients=[args.gpg])
