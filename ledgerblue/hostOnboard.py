...
if __name__ == "__main__":
    ...
    def enter_if_none_and_normalize(hint, strg):
        if (
            strg is None
        ):  # or len(string) == 0: len 0 is accepted, to specify without being bothered by a message
    ...
    args.passphrase = enter_if_none_and_normalize(
        "Derivation passphrase: ", args.passphrase
    )
    ...
    if args.passphrase:
        apdudata += bytearray([len(args.passphrase)]) + bytearray(
            args.passphrase, "utf8"
        )
    else:
        apdudata += bytearray([0])
