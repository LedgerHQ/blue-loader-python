...
def get_argparser():
    parser = argparse.ArgumentParser(
        description="Load an app onto the device from a hex file."
    )
    parser.add_argument(
        "--targetId",
    ...
    parser.add_argument(
        "--apiLevel",
        help="Set the API level of the SDK used to build the app",
        type=auto_int,
        default=-1,
    )
    parser.add_argument(
        "--fileName",
        help="The application hex file to be loaded onto the device",
        required=True,
    )
...
"ed25519" or "bls12381g1"), can be repeated""",
    ...
    parser.add_argument(
        "--path",
        help="""A BIP 32 path to which derivation is locked (format decimal a'/b'/c), can be
repeated""",
        action="append",
    )
    parser.add_argument(
        "--path_slip21",
        help="""A SLIP 21 path to which derivation is locked""",
        action="append",
    )
    parser.add_argument(
        "--appName",
        help="The name to give the application after loading it",
        required=True,
    )
    parser.add_argument(
        "--signature", help="A signature of the application (hex encoded)"
    )
    parser.add_argument(
        "--signApp",
        help="Sign application with provided signPrivateKey",
        action="store_true",
    )
    parser.add_argument(
        "--appFlags", help="The application flags", type=auto_int, default=0
    )
    parser.add_argument(
        "--bootAddr", help="The application's boot address", type=auto_int
    )
    parser.add_argument(
        "--rootPrivateKey",
...
a random one will be generated)""",
    ...
    parser.add_argument(
        "--signPrivateKey", help="Set the private key used to sign the loaded app"
    )
    ...
    parser.add_argument(
        "--params",
        help="Store icon and install parameters in a parameter section before the code",
        action="store_true",
    )
    parser.add_argument(
        "--tlv",
        help="Use install parameters for all variable length parameters",
        action="store_true",
    )
    parser.add_argument(
        "--dataSize",
        help="The code section's size in the provided hex file (to separate data from code, if not provided the whole allocated NVRAM section for the application will remain readonly.",
        type=auto_int,
    )
    parser.add_argument("--appVersion", help="The application version (as a string)")
    parser.add_argument(
        "--offline",
        help="Request to only output application load APDUs into given filename",
    )
    parser.add_argument(
        "--offlineText",
        help="Request to only output application load APDUs into given filename in text mode",
        action="store_true",
    )
    parser.add_argument(
        "--installparamsSize",
        help="The loaded install parameters section size (when parameters are already included within the .hex file.",
        type=auto_int,
    )
    parser.add_argument(
        "--tlvraw",
        help="Add a custom install param with the hextag:hexvalue encoding",
        action="append",
    )
    parser.add_argument(
        "--dep", help="Add a dependency over an appname[:appversion]", action="append"
    )
    parser.add_argument(
...
def main(args, debug: bool = True):
    ...
    if args.tlv:
        ...
        if args.tlvraw:
            for tlvraw in args.tlvraw:
                ...
                installparams += encodetlv(
                    int(hextag, 16), binascii.unhexlify(hexvalue)
                )

        if (not (args.appFlags & 2)) and (
            args.installparamsSize is None or args.installparamsSize == 0
        ):
            ...
            if not args.appVersion is None:
                installparams += encodetlv(
                    BOLOS_TAG_APPVERSION, string_to_bytes(args.appVersion)
                )
            if not args.icon is None:
            ...
            param_start = (
                printer.maxAddr()
                + (PAGE_ALIGNMENT - (args.dataSize % PAGE_ALIGNMENT)) % PAGE_ALIGNMENT
            )
    ...
    if signature == None and args.signApp:
        ...
        signature = masterPrivate.ecdsa_serialize(
            masterPrivate.ecdsa_sign(bytes(binascii.unhexlify(hash)), raw=True)
        )
        if debug:
