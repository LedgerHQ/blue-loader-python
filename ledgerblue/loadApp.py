...
a random one will be generated)""")
    ...
    parser.add_argument("--dataSize",
                        help="The code section's size in the provided hex file (to separate data from code, if "
                             "not provided the whole allocated NVRAM section for the application will remain readonly.",
                        type=auto_int)
...
if __name__ == '__main__':
    ...
    # tlv mode does not support explicit by name removal,
    # would require a list app before to identify the hash to be removed
    if (not (args.appFlags & 2)) and args.delete:
        loader.deleteApp(args.appName)

    if args.tlv:
        # if code length is not provided, then consider the whole
        # provided hex file is the code and no data section is split
        ...
        if (not (args.appFlags & 2)) and (args.installparamsSize is None or args.installparamsSize == 0):
            ...
            # only append install param section when not an upgrade as
            # it has already been computed in the encrypted and signed chunk
        ...
        # take care of aligning the parameters sections to avoid
        # possible invalid dereference of aligned words in the program nvram.
        ...
        loader.createAppNoInstallParams(args.appFlags, appLength, args.appName, None, path, 0,
                                        len(paramsSectionContent), args.appVersion)
    else:
        # account for added regions (install parameters, icon ...)
        appLength = printer.maxAddr() - printer.minAddr()
        loader.createAppNoInstallParams(args.appFlags, appLength, args.appName, args.icon, path, None, None,
                                        args.appVersion)
    ...
    if signature is None and args.signApp:
