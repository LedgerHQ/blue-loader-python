...
class HexLoader:
    ...
    def crcSegment(self, offsetSegment, lengthSegment, crcExpected):
        data = b'\x08' + struct.pack('>H', offsetSegment) + struct.pack('>I', lengthSegment) + struct.pack('>H',
                                                                                                           crcExpected)
        self.exchange(self.cla, 0x00, 0x00, 0x00, data)
    ...
    def createAppNoInstallParams(self, appflags, applength, appname, icon=None, path=None, iconOffset=None,
                                 iconSize=None, appversion=None):
        data = b'\x0B' + struct.pack('>I', applength) + struct.pack('>I', appflags) + struct.pack('>B', len(
            appname)) + appname
    ...
    def getVersion(self):
        ...
        result = {}
        offset = 0
        result['targetId'] = (response[offset] << 24) | (response[offset + 1] << 16) | (response[offset + 2] << 8) | \
                             response[offset + 3]
        offset += 4
        result['osVersion'] = response[offset + 1: offset + 1 + response[offset]].decode('utf-8')
        offset += 1 + response[offset]
        offset += 1
        result['flags'] = (response[offset] << 24) | (response[offset + 1] << 16) | (response[offset + 2] << 8) | \
                          response[offset + 3]
        offset += 4
