...
def get_argparser():
    parser = argparse.ArgumentParser(
        description="""Generate an attestation keypair, using the provided Owner private
key to sign the Owner Certificate."""
    ...
    parser.add_argument(
        "--certificate",
        help="""Optional certificate to store if finalizing the endorsement (hex
encoded), if no private key is specified""",
    )
    parser.add_argument(
        "--privateKey",
        help="""Optional private key to use to create a test certificate (hex encoded),
if no certificate is specified""",
    ...
    parser.add_argument(
        "--issuerKey",
        help="Issuer key (hex encoded, default is batch 1)",
...
def getDeployedSecretV2(dongle, masterPrivate, targetid, issuerKey):
    ...
    certificate = (
        bytearray([len(testMasterPublic)])
        + testMasterPublic
        + bytearray([len(signature)])
        + signature
    )
    apdu = (
        bytearray([0xE0, 0x51, 0x00, 0x00])
    ...
    while True:
        ...
        offset += certificate[offset - 1] + 1
        certificateSignatureArray = certificate[
            offset : offset + certificate[offset - 1]
        ]
        certificateSignature = last_pub_key.ecdsa_deserialize(
            bytes(certificateSignatureArray)
        )
        # first cert contains a header field which holds the certificate's public key role
        if index == 0:
            certificateSignedData = (
                bytearray([0x02]) + certificateHeader + certificatePublicKey
            )
            # Could check if the device certificate is signed by the issuer public key
        # ephemeral key certificate
        else:
            certificateSignedData = (
                bytearray([0x12]) + deviceNonce + nonce + certificatePublicKey
            )
        if not last_pub_key.ecdsa_verify(
            bytes(certificateSignedData), certificateSignature
        ):
...
if __name__ == "__main__":
    ...
    if not args.rootPrivateKey:
        ...
        args.issuerKey = binascii.hexlify(
            issuerPrivKey.pubkey.serialize(compressed=False)
        ).decode("utf-8")

    dongle = getDongle(args.apdu)
    print(args.rootPrivateKey)
    publicKey = getDeployedSecretV2(
        dongle, bytearray.fromhex(args.rootPrivateKey), args.targetId, args.issuerKey
    )
